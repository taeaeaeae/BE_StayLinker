...
class CommunityReplyController(
    ...
    override fun createReply(
        ...
        ResponseEntity.status(HttpStatus.OK).body(communityReplyService.createReply(replyRequest, id))
    ...
    override fun updateReply(
        ...
        ResponseEntity.status(HttpStatus.OK).body(communityReplyService.updateReply(replyRequest, id))

    @Operation(summary = "커뮤니티 댓글 삭제 API", description = "커뮤니티 댓글 id 값을 넣어 주시면 됩니다")
    @DeleteMapping("/{id}")
    override fun deleteReply(
        @PathVariable id: Long,
    ): ResponseEntity<DefaultResponse> = ResponseEntity.status(HttpStatus.OK).body(communityReplyService.deleteReply(id))

    @Operation(summary = "커뮤니티 댓글 신고 API", description = "커뮤니티 댓글 id 값을 넣어 주시면 됩니다")
    @PatchMapping("/report/{id}")
    override fun reportReply(
        @PathVariable id: Long
    ): ResponseEntity<DefaultResponse> = ResponseEntity.status(HttpStatus.OK).body(communityReplyService.reportReply(id))
}